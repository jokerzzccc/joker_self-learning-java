...
![图片](https://2021-joker.oss-cn-shanghai.aliyuncs.com/java_img/642.png)
...
![图片](https://2021-joker.oss-cn-shanghai.aliyuncs.com/java_img/640.png)
...
![图片](https://2021-joker.oss-cn-shanghai.aliyuncs.com/java_img/641.png)
...
1. DispatcherServlet表示前置控制器，是整个SpringMVC的控制中心。用户发出请求，DispatcherServlet接收请求并拦截请求。
   ...
   **如上url拆分成三部分：**
   ...
   通过分析，如上url表示为：请求位 于服务器localhost:8080上的SpringMVC站点的hello控制器。
...
- SpringMVC 入门 helloSpring 的实现

## 2.1 xml配置实现

- 配置实现 用于理解SpringMVC 的原理，
...
9、将自己的类交给SpringIOC容器，注册bean,如上
...
小结：看这个估计大部分同学都能理解其中的**原理**了，但是我们实际开发才不会这么写，不然就疯了，还学这个玩意干嘛！我们来看个注解版实现，这才是SpringMVC的精髓，
...
8. 编写一个Java控制类：com.joker.controller.HelloController , 注意编码规范
...
4. 编写完毕后，去Spring配置文件中注册请求的bean；name对应请求路径，class对应处理请求的类
   ...
   ```xml
   <bean name="/t1" class="com.joker.controller.ControllerTest1"/>
   ```
...
2、提交数据 : http://localhost:8080/mvc04/user?name=joker&id=1&age=15
...
后台输出 : User { id=1, name='joker', age=15 }
...
- 知道使用第三方依赖就好，jackson 或者 fastjson
...
- 异步，无刷新请求
- JQuery 就是一个库：有js的大量函数（方法）

- 要求：html + css :略懂+js （超级熟练）
...
![](https://2021-joker.oss-cn-shanghai.aliyuncs.com/java_img/ajax.png)

- 其实，就是后端只负责传递一个data，ajax把主动权交给了前端

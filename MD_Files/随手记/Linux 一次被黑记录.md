...
1. 查看进程 cpu 占用 使用 top，有将近100%的，它*******
...
4. 删除定时任务
   ...
   ```sh
   # 如果直接，把 /var/spool/cron/root 使用 chattr -i 解锁，发现，还是无法 rm 删除掉，就需要解除它上级文件夹的锁，如下。
   # 清除crontab
   cd /var/spool/
   chattr -i cron/
   # 这就是解除定时任务，或者 rm /var/spool/cron/root
   crontab -r 
   ```

5. 删除相关文件。进入到 /etc 目录下；都要使用 chattr -i 解锁
...
6. 因为我的 /root/.ssh/authorized_keys 是没有东西的原本，所以，我直接把这个文件删 了，这里有入侵的 ssh 密钥
...
7. 修复 SElinux和wget、curl指令。因为从脚本里可以看到，它把这些改了

   ```sh
   # SElinux 文件/etc/selinux/config 改值为 enforcing
...
找到 自己redis 配置文件 redis.conf 的目录 修改 密码，
...
- 由病毒脚本最后可知，可以看见输出了日志 到 /var/log/ 下面，可以查看下，cron的日志文件，就可以看到有这个 cron.daily
